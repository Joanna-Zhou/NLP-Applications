...
class EncoderDecoder(EncoderDecoderBase):
    ...
    def update_beam(self, htilde_t, b_tm1_1, logpb_tm1, logpy_t):
        ...
        if self.cell_type == 'lstm':
            # (N, K, 2H) -> (NK, 2H) -> (N, K, 2H)
            b_t_0 = (htilde_t[0].view(NK, -1).index_select(0, b_t_id).view(N, K, -1),
                     htilde_t[1].view(NK, -1).index_select(0, b_t_id).view(N, K, -1))
        else:
        ...
        # (t+1, NK) -> (t+1, N, K)
        b_t_1 = torch.cat(
            (b_tm1_selected, v_id.reshape(1, NK)), -1).view(t, N, K)
